...
Preserve the receiver when invoking real Supabase client methods:
...
For teardown, search a bounded number of pages for the exact run-owned email, delete only the returned exact user ID, and fail the hook if a successful signup cannot be found and deleted. Delete Mailpit messages only by the IDs collected for the exact run recipient.

Database tests should consume `TEST_DATABASE_URL` first and keep only a repository-relative CLI fallback.

## Why This Works

The env-first path keeps credentials process-only in CI and restricted sandboxes. The bounded CLI fallback makes a normal clean checkout work after local Supabase starts without persisting credentials. Receiver binding matches how the Supabase client stores initialization and PKCE state. Fail-closed, ownership-scoped cleanup prevents false success and cross-run deletion.
