...
**B3 워드헌트 스케줄러** — `5cbdd9a`. 발견 횟수 마일스톤 → 룰셋 시계(16-22초, 34-42초,
SPECIAL은 정확히 `finalRushStartsAtMs`). 창 안의 순간은 콘텐츠 키에서 파생한다 — 매치는
서버가 스케줄을 주지만 연습판은 자기 고스트와 겨루므로, 같은 보드가 두 번 다르게 굴면
비교가 소음이 된다.

#### 그런데 워드헌트를 가진 팩이 80개 중 5개뿐이다

```
content/learning/word-hunts.curated.v1.json  →  2팩
손으로 만든 프리뷰 팩                          →  3팩
나머지 77개 생성 팩                            →  없음
```

스케줄러는 맞게 돌지만 **대부분의 보드는 워드헌트를 한 번도 보지 못한다.** 드래프트의
헌트 좌표는 쓸 수 없어서(위 표 참조) 큐레이션 파일에만 의존하는데, 그게 2팩이다.
**다음에 이 루프의 체감을 올리려면 코드가 아니라 큐레이션이 필요하다** — `en-paradox-labyrinth`,
`en-tranquility-garden` 두 팩이 형식의 본보기다.

### 다음 순서 (계획서 기준)

1. **워드헌트 큐레이션 확대** — B3가 실제로 느껴지려면 이게 선행이다 (콘텐츠 작업)
