...
들어가 있고, 게이트는 그 커밋된 트리에서 초록이다. 두 브랜치 모두 origin에 푸시했다.**
...
원격    두 브랜치 모두 origin과 일치 (e322e16 / f39ced6)
...
1. **7월 브랜치 2개를 살릴지 정한다.** `agent/*` 6개와 `codex/*` 3개(2026-07-16 ~ 07-30)를
   전부 푸시했으니 유실 위험은 없다. `git cherry`로 재보니 그중 **7개는 패치 내용이 이미
   릴리스 브랜치에 있다** — 지워도 잃을 것이 없다. 실제로 반영 안 된 것은 둘뿐이다:
   `codex/supabase-auth-integration`(167파일, +16,565 — 게이트·검증 도구)과
   `codex/physical-device-guest-game`(45파일, +1,580 — 기기 증거·플레이어블 레지스트리).
   릴리스 브랜치는 이 도구들을 **어디서도 참조하지 않으므로** 끊어진 링크는 없고, 26단계
   게이트는 이것들 없이 초록이다. 즉 지금 운영에 필요한 것은 아니고, 되살릴 가치가 있는
   작업인지가 남은 판단이다.
2. **처분표 승인** (위 두 곳).
