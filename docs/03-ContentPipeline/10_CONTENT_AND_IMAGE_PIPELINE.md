...
- `PrivateGameSolutionV1`: 차이점·단어 찾기·서든데스 hitbox, 최종 정답·별칭·힌트 단위·저작된 5단 힌트 사다리·정답 option ID를 포함한다. 클라이언트 카탈로그에 노출하지 않는다. <!-- REQ: CONTENT-003 -->
...
`finalChallenge.hintUnits`와 `finalChallenge.hintLadder`는 서로 다른 계약이다.

- `hintUnits`는 정규화된 canonical answer를 `Intl.Segmenter` grapheme 경계로 정확히 나눈 배열이다. 공백과 문장부호도 화면에 보이는 원래 위치를 보존한다.
- `hintLadder`는 교육 검토자가 미리 쓴 정확히 다섯 개의 `HintStepV1`이다. 각 단계는 `ordinal`, 설계에 고정된 `kind`, 필수 `ko`/`en` 문구, grapheme 또는 option index, 고정 ranked penalty 1만 가진다.
- 런타임은 단계나 교육 문구를 생성하지 않는다. 입수 시점에만 segmentation과 카테고리 규칙을 검증하고, Expo 소비자는 저장된 단위와 패턴을 렌더링한다.
- reveal index는 공백·문장부호를 가리킬 수 없다. 5단계 전 정답 전체 공개, 중복 index, 짧은 영어 단어의 양 끝 조기 공개, 틀린 초성, 검토되지 않은 한자를 거부한다.
- Hanja 문구는 `reviewedHanja`와 `hanjaReviewStatus: "APPROVED"`가 함께 있을 때만 허용한다. 승인 증거가 없는 Hanja를 추론하거나 자동 보충하지 않는다.
- 일반상식의 `ELIMINATE_OPTION` index는 오답만 가리켜야 하며 중복 제거와 모든 오답 제거를 거부한다.

프로덕션 흐름의 단일 원천은 `catalog.v1.json -> drafts -> geometry/evidence -> manifest.v1.json -> mobile registry`이다. bundle writer는 `hintUnits`와 저작된 `hintLadder`를 별도로 기록한다. manifest는 각 bundle의 `hintLadderAdmission.{status,stepCount,errors,hash}`와 `rankedEligible`을 기록한다. 정확히 다섯 단계가 입수되고 canonical ladder hash가 있는 bundle만 ranked pin 후보가 될 수 있다. `publishBlocked` 권리·교육 승인 게이트는 이 힌트 적격성과 별개로 계속 적용된다.
...
9. 카테고리별 5단 힌트 사다리, 조기 정답 공개, localization, grapheme/option index, Hanja 승인 상태를 검사한다.
10. 공개 자산·로컬 locator·권리 entry의 SHA-256 bijection을 검사한다. 권리·교육의 사람 승인은 이 로컬 검증과 분리된 외부 게시 blocker다. <!-- REQ: CONTENT-020 -->
