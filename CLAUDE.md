...
## 일 처리 방식 — 끝까지 하고, 못 하는 것만 넘긴다

기본값은 **에이전트가 끝낸다.** "이건 사람이 해야 합니다"로 넘기는 것은 규칙상 대신할 수 없는
것뿐이고, 그때도 빈손으로 넘기지 않는다.

### 에이전트가 하는 것

CLI 실행, 파일 편집, 마이그레이션 승격, 콘텐츠 게시, 빌드, 게이트 확인. 브라우저·Windows MCP로
화면을 열어 **상태를 읽고 확인하는 것**. 사람이 알려준 값을 설정 파일에 기록하고 재생성하는 것.
스토어 등록정보·설문 답안처럼 **붙여넣기만 하면 되는 문안을 미리 만들어 두는 것**.

"확인이 필요하다"고 말하기 전에, **직접 재볼 수 있는 것인지 먼저 본다.** 이 저장소에서 인수인계
문서의 차단 원인이 실제와 달랐던 적이 있다.

### 사람에게 넘기는 것

| 항목 | 이유 |
| --- | --- |
| 계정 생성, 로그인 비밀번호 입력 | 계정 생성과 인증은 대신하지 않는다 |
| 결제, 카드·신분증 입력 | 금융·신분 정보는 어떤 필드에도 넣지 않는다 |
| API 키·비밀번호를 평문으로 다루기 | 시크릿은 사람의 셸과 비밀번호 관리자에만 있다 |
| 약관 동의, OAuth 권한 부여 | 동의는 사람이 한다 |
| 개인정보가 들어간 폼 제출, 되돌릴 수 없는 버튼 | 제출·게시·삭제는 사람이 누른다 |

### 넘길 때의 형식

1. 에이전트가 **정확한 페이지를 열어준다.** "구글에서 찾아보세요"가 아니라 그 화면까지.
2. **지금 무엇이 보이고 어디를 눌러야 하는지** 짚는다. 화면을 읽어서 확인한다.
3. 넣을 값은 **미리 준비해 둔다.** 사람은 붙여넣기만 한다.
4. 사람이 누른 뒤 **에이전트가 결과를 검증**하고 다음 단계로 잇는다.

시크릿은 에이전트를 거치지 않는다. 사람이 자기 셸이나 콘솔에 직접 넣고, 에이전트에게는
**비밀이 아닌 식별자만** 알려준다 — 프로젝트 ref, 배포 origin, 인증서 지문 같은 것.

출시 절차 전체는 [`open-manual.md`](open-manual.md)와
[`docs/runbooks/google-play-release.md`](docs/runbooks/google-play-release.md)에 있다.
