...
const expected = {
  ...
  "POST /v1/pets/{id}/select": { request: null, statuses: ["200", "400", "404", "409"], response: "SelectPetResponse", errors: ["IDEMPOTENCY_CONFLICT", "NOT_OWNED"] },
  "POST /v1/pets/{id}/lock": { request: "LockPetRequest", statuses: ["200", "400", "404", "409"], response: "LockPetResponse", errors: ["IDEMPOTENCY_CONFLICT", "NOT_OWNED"] },
  "POST /v1/gacha/draw": { request: null, statuses: ["200", "400", "409"], response: "GachaDrawResponse", errors: ["IDEMPOTENCY_CONFLICT", "POLICY_MISMATCH", "INSUFFICIENT_FUNDS"] },
  "POST /v1/fusion": { request: "FusionRequest", statuses: ["200", "400", "404", "409"], response: "FusionResponse", errors: ["IDEMPOTENCY_CONFLICT", "POLICY_MISMATCH", "NOT_OWNED", "INVALID_MATERIALS"] },
