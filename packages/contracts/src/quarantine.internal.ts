import { PRIVACY_OPERATOR_ROLE, type QuarantinePolicyV1, type QuarantineStatus } from './quarantine.js';
...
type QuarantineReceipt={jobId:string;status:QuarantineStatus;checkpoint:number;affectedFieldCount:number};
type QuarantineLease={jobId:string;workerId:string;fence:number};
...
/** @internal Test-only persistence adapter. It is intentionally absent from package exports and the public barrel. */
...
/** @internal Test-only repository/capability facade. */
export class MemoryQuarantineRepository {
 constructor(private readonly store=new MemoryQuarantineStore()){}
 private load(){return this.store.load();} private save(records:Map<string,QuarantineRecord>){this.store.save(records);}
