import {expect,it} from 'vitest';import {projectMatchEvent,projectSnapshot} from './projection.js';import {matchSnapshotV1Schema,serverEventEnvelopeSchema} from './socket.schema.js';import type {MatchEvent,MatchStateV1} from './match.js';import {createTestingReplayBundle} from '../../game-engine/src/testing-fixtures.js';
...
it('projects only the current learning hint to its owner and redacts it for every other viewer',()=>{
 const event={type:'HINT_STEP_REVEALED',eventId:'e',matchId:'123e4567-e89b-12d3-a456-426614174000',eventSeq:8,causedByCommandSeq:2,stateRevision:5,occurredAtMs:2,phase:'PLAYING',payload:{playerId:'p1',requestId:'123e4567-e89b-42d3-a456-426614174000',ordinal:4,kind:'VISUAL_REGION',localizedText:'Look toward the top left',publicPattern:'c__',publicRegion:{kind:'REGION',imageSide:'A',region:'TOP_LEFT'},rankedPenaltyUnits:1,cumulativeRankedPenaltyUnits:4,coachChargesRemaining:null}} satisfies MatchEvent;
 const owner=projectMatchEvent(event,{} as MatchStateV1,'p1');
 expect(owner).toMatchObject({type:'hint_step_revealed',payload:{requestId:event.payload.requestId,ordinal:4,localizedText:event.payload.localizedText,publicRegion:event.payload.publicRegion,cumulativeRankedPenaltyUnits:4,coachChargesRemaining:null}});
 expect(serverEventEnvelopeSchema.safeParse(owner).success).toBe(true);
 expect(projectMatchEvent(event,{} as MatchStateV1,'p2')).toMatchObject({type:'state_advanced',eventSeq:8,payload:{redacted:true}});
 const serialized=JSON.stringify(owner);
 expect(serialized).not.toMatch(/playerId|remainingSteps|canonicalAnswer|privateSolution|hitboxes|imageB|cx|cy|radius/i);
});
it('projects an authoritative owner learning cursor and null for opponent or spectator',()=>{
 ...
 expect(JSON.stringify(spectator)).not.toMatch(/Current 2|processedRequestIds/);
});
it('projects viewer snapshot without private solution material',()=>{const state={matchId:'00000000-0000-4000-8000-000000000001',engineVersion:'e',rulesetVersion:'1.0.0',rulesetHash:'a'.repeat(64),contentRevisionId:'00000000-0000-4000-8000-000000000002',contentHash:'b'.repeat(64),assetPolicyVersion:'1.0.0',assetLoadDeadlineMs:20,expectedAssets:[{side:'A',url:`https://cdn.example/assets/${'c'.repeat(64)}.png`,sha256:'c'.repeat(64),encodedBytes:10,width:10,height:10,mimeType:'image/png'},{side:'B',url:`https://cdn.example/assets/${'d'.repeat(64)}.webp`,sha256:'d'.repeat(64),encodedBytes:10,width:10,height:10,mimeType:'image/webp'}],phase:'PLAYING',phaseEndsAtMs:30,stateRevision:4,nextEventSeq:8,players:[{playerId:'p1',assetLoadStatus:'READY',score:2,wrongFinalAttempts:1,hintCredits:2,revealedHintIndexes:[0],publicPattern:'c_t',answerUntilMs:null},{playerId:'p2',assetLoadStatus:'READY',score:1,wrongFinalAttempts:0,hintCredits:0,revealedHintIndexes:[],publicPattern:null,answerUntilMs:null}],connections:[{playerId:'p1',status:'CONNECTED',disconnectEpoch:0,forfeitAtMs:null},{playerId:'p2',status:'CONNECTED',disconnectEpoch:0,forfeitAtMs:null}],objectives:[],activeMission:null,finalChallenge:{unlockedAtMs:10,unlockSource:'TIME'},meaningQuizzes:[],suddenDeath:null,winnerPlayerId:null,endReason:null,privateSolution:{finalChallenge:{canonicalAnswer:'cat',aliases:['kitty'],meaning:{prompt:'meaning?',options:[{id:'a',label:'A'},{id:'b',label:'B'}],correctOptionId:'a'}},differences:[],suddenDeath:{objectiveId:'sd',hitboxes:{imageA:{cx:.1,cy:.1,r:.1},imageB:{cx:.1,cy:.1,r:.1}}}}} as unknown as MatchStateV1;const out=projectSnapshot(state,'p1',15);expect(matchSnapshotV1Schema.safeParse(out).success).toBe(true);expect(out.lastEventSeq).toBe(7);expect(out.finalChallenge.viewer.publicPattern).toBe('c_t');expect(forbiddenPaths(out)).toEqual([]);expect(JSON.stringify(out)).not.toMatch(/canonicalAnswer|aliases|correctOptionId|kitty|privateSolution/)});
...
it('maps every domain event for owner, opponent, and public visibility without leaking private keys',()=>{
 const base={eventId:'e',matchId:'00000000-0000-4000-8000-000000000001',eventSeq:8,causedByCommandSeq:1,stateRevision:5,occurredAtMs:10,phase:'PLAYING' as const};
 const events=[
  ...
  {...base,type:'OBJECTIVE_CLAIMED',payload:{objectiveId:'d1',ownerPlayerId:'p1',kind:'DIFFERENCE' as const}},
 ...
 for(const event of events)for(const viewer of ['p1','p2','public']){
  ...
  if(expected==='state_advanced')expect(projected.payload).toEqual({redacted:true});
 }
});
