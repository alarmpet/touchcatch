import type { HintKind, PrivateGameSolutionV1, PublicHintRegionV1 } from './content.js';
import type { CoachArchetype, PetRarity } from './pet-catalog.js';
...
export type LearningHintOrdinal=1|2|3|4|5;
export type SelectedHintPetV1={rarity:PetRarity;level:number;coachArchetype:CoachArchetype};
export type LearningHintPlayerConfigV1={mode:LearningHintMode;selectedPet:SelectedHintPetV1|null};
export type LearningHintPlayerStateV1=LearningHintPlayerConfigV1&{coachChargesRemaining:number;revealedOrdinals:ReadonlyArray<LearningHintOrdinal>;cumulativeRankedPenaltyUnits:number;processedRequestIds:ReadonlyArray<string>};
export type CreateMatchInitialStateInput={matchId:string;createdAtMs:number;engineVersion:string;rulesetHash:string;playerIds:readonly [string,string];contentManifest:PinnedMatchContentManifestV1;privateSolution:PrivateGameSolutionV1;randomSchedule:MatchRandomScheduleV1;learningHintPlayers?:readonly [LearningHintPlayerConfigV1|null,LearningHintPlayerConfigV1|null]};
export type MatchPlayerStateV1={playerId:string;assetLoadStatus:'PENDING'|'READY'|'FAILED';assetFailure:{reason:'FETCH_FAILED';assetHash:string;attempts:2}|{reason:'ATTESTATION_MISMATCH'}|{reason:'TIMEOUT'}|null;assetAttestation:{contentHash:string;assetHashes:ReadonlyArray<string>;decodedDimensions:ReadonlyArray<{assetHash:string;width:number;height:number}>}|null;score:number;wrongFinalAttempts:number;hintCredits:number;revealedHintIndexes:ReadonlyArray<number>;publicPattern:string|null;learningHints:LearningHintPlayerStateV1|null;finalAnswerStatus:'NOT_SUBMITTED'|'MEANING_PENDING'|'FAILED'|'SETTLED';meaningCorrect:boolean|null;answerUntilMs:number|null;tapRateWindow:{windowIndex:number|null;acceptedCount:number}};
export type MatchStateV1={matchId:string;engineVersion:string;rulesetVersion:'1.0.0';rulesetHash:string;contentRevisionId:string;contentLanguage:'ko'|'en'|'ja';contentHash:string;assetPolicyVersion:'1.0.0';createdAtMs:number;assetLoadDeadlineMs:number;expectedAssets:readonly [ExpectedMatchAssetV1,ExpectedMatchAssetV1];phase:MatchPhase;phaseEndsAtMs:number|null;startedAtMs:number|null;gameplayClosesAtMs:number|null;settlementCapAtMs:number|null;stateRevision:number;nextEventSeq:number;players:readonly [MatchPlayerStateV1,MatchPlayerStateV1];connections:readonly [{playerId:string;status:'CONNECTED'|'DISCONNECTED';disconnectEpoch:number;disconnectedAtMs:number|null;forfeitAtMs:number|null},{playerId:string;status:'CONNECTED'|'DISCONNECTED';disconnectEpoch:number;disconnectedAtMs:number|null;forfeitAtMs:number|null}];objectives:ReadonlyArray<{objectiveId:string;kind:'DIFFERENCE'|'WORD_HUNT';ownerPlayerId:string|null}>;activeMission:{missionId:string;kind:'NORMAL'|'SPECIAL';publicPrompt:string;startedAtMs:number;endsAtMs:number}|null;finalChallenge:{unlockedAtMs:number|null;unlockSource:'TIME'|'DIFFERENCE'|'WORD_HUNT'|null};meaningQuizzes:ReadonlyArray<{playerId:string;quizOrdinal:number;startedAtMs:number;endsAtMs:number;submitted:boolean}>;suddenDeath:{objectiveId:string;endsAtMs:number}|null;randomSchedule:MatchRandomScheduleV1;privateSolution:PrivateGameSolutionV1;winnerPlayerId:string|null;endReason:MatchEndReason|null};
export type MatchInitialStateV1=MatchStateV1&{phase:'WAITING_FOR_ASSETS';stateRevision:0;nextEventSeq:1};
type Base={commandId:string;matchId:string;commandSeq:number;receivedAtMs:number};
export type PlayerCommandPayload={type:'READY';contentRevisionId:string;contentHash:string;assetHashes:ReadonlyArray<string>;decodedDimensions:ReadonlyArray<{assetHash:string;width:number;height:number}>}|{type:'REPORT_ASSET_LOAD_FAILURE';assetHash:string;attempts:2}|{type:'TAP_IMAGE';imageSide:'A'|'B';x:number;y:number}|{type:'SUBMIT_FINAL_ANSWER';answer:string}|{type:'SUBMIT_MEANING';optionId:string}|{type:'USE_HINT'}|{type:'USE_LEARNING_HINT';expectedOrdinal:number};
export type TimerCommandPayload={type:'ASSET_LOAD_TIMEOUT'}|{type:'START_MATCH'}|{type:'START_WORD_HUNT';missionId:string}|{type:'END_WORD_HUNT';missionId:string}|{type:'UNLOCK_FINAL_CHALLENGE'}|{type:'START_FINAL_RUSH'}|{type:'CLOSE_INPUT'}|{type:'ANSWER_LOCK_EXPIRED';playerId:string;wrongAttemptOrdinal:number}|{type:'MEANING_TIMEOUT';playerId:string;quizOrdinal:number}|{type:'DISCONNECT_FORFEIT_TIMEOUT';playerId:string;disconnectEpoch:number}|{type:'SUDDEN_DEATH_TIMEOUT';objectiveId:string};
...
export type MatchEvent=EventBase&(
 ...
 |{type:'HINT_STEP_REVEALED';payload:{playerId:string;requestId:string;ordinal:LearningHintOrdinal;kind:HintKind;localizedText:string;publicPattern:string;publicRegion:PublicHintRegionV1|null;rankedPenaltyUnits:0|1;cumulativeRankedPenaltyUnits:number;coachChargesRemaining:number|null}}
...
export type DomainDecision={status:'APPLIED'}|{status:'REJECTED';reason:'REVISION_AHEAD'|'ALREADY_READY'|'ALREADY_CLAIMED'|'INPUT_LOCKED'|'NO_HINT_CREDIT'|'INVALID_HINT_LADDER'|'HINT_ORDINAL_CONFLICT'|'NO_HINT_REMAINING'|'RATE_LIMITED'|'OBSOLETE_TIMER'|'OBSOLETE_SYSTEM_COMMAND'|'MATCH_INPUT_CLOSED'|'NOT_A_PARTICIPANT'};
