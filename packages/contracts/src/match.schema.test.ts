...
describe('stable TIMER and SYSTEM command identities',()=>{const matchId='00000000-0000-4000-8000-000000000001';const timers=[{type:'ASSET_LOAD_TIMEOUT'},{type:'START_MATCH'},{type:'START_WORD_HUNT',missionId:'w1'},{type:'END_WORD_HUNT',missionId:'w1'},{type:'UNLOCK_FINAL_CHALLENGE'},{type:'START_FINAL_RUSH'},{type:'CLOSE_INPUT'},{type:'ANSWER_LOCK_EXPIRED',playerId:'p1',wrongAttemptOrdinal:1},{type:'MEANING_TIMEOUT',playerId:'p1',quizOrdinal:1},{type:'DISCONNECT_FORFEIT_TIMEOUT',playerId:'p1',disconnectEpoch:1},{type:'SUDDEN_DEATH_TIMEOUT'}];it.each(timers)('rejects mismatched TIMER id for $type',(payload)=>{const id=deriveTimerId(matchId,payload);const base={source:'TIMER',commandId:id,timerId:id,matchId,commandSeq:1,receivedAtMs:1,dueAtMs:1,payload};expect(parseMatchCommandV1(base).commandId).toBe(id);expect(()=>parseMatchCommandV1({...base,commandId:`${id}x`,timerId:`${id}x`})).toThrow(/stable id/);});const systems=[{type:'PLAYER_CONNECTION_CHANGED',playerId:'p1',disconnectEpoch:1,status:'DISCONNECTED'},{type:'CANCEL_NO_CONTEST',incidentId:'incident',reason:'SERVER_OWNERSHIP_LOST'}];it.each(systems)('rejects mismatched SYSTEM id for $type',(payload)=>{const id=deriveSystemCommandId(matchId,payload);const base={source:'SYSTEM',commandId:id,systemCommandId:id,matchId,commandSeq:1,receivedAtMs:1,payload};expect(parseMatchCommandV1(base).commandId).toBe(id);expect(()=>parseMatchCommandV1({...base,commandId:`${id}x`,systemCommandId:`${id}x`})).toThrow(/stable id/);});});
