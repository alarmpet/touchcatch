...
export const CONTENT_INTEGRATION_EVIDENCE = Object.freeze({
  ...
  verifyWire(value:unknown){const parsed=clientCommandEnvelopeSchema.parse(value);if(parsed.payload.type!=='SUBMIT_FINAL_ANSWER')throw Error('answer command required');const accepts=(answer:string)=>clientCommandEnvelopeSchema.safeParse({...parsed,payload:{type:'SUBMIT_FINAL_ANSWER' as const,answer}}).success;return {normalizedAnswer:normalizeFinalAnswer(parsed.payload.answer),acceptedRaw128AfterNormalization:accepts(`Ａ${' '.repeat(127)}`),rejectedRaw129:!accepts(`a${' '.repeat(128)}`),rejectedNormalized65:!accepts('a'.repeat(65)),acceptedUtf8Bytes256:accepts('😀'.repeat(64)),rejectedUtf8Bytes260:!accepts('😀'.repeat(65))};},
