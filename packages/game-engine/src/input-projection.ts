...
export function derivePlayerInputState(state:ProjectionState,playerId:string,now:number,rules:ProjectionRules,connection:'CONNECTED'|'RECONNECTING'):PlayerInputState {
 ...
 const complete=p.finalAnswerStatus==='FAILED'||p.finalAnswerStatus==='SETTLED';const answer=complete?'COMPLETED':connection==='RECONNECTING'||quiz||reveal||state.finalChallenge.unlockedAtMs===null||isTimedInputLocked(p.answerUntilMs,now)||!['PLAYING','FINAL_RUSH'].includes(state.phase)?'LOCKED':'ENABLED';
