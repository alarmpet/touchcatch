...
it('maps every terminal reason exhaustively',()=>{expect(['SCORE_TARGET','TIMEOUT_TIEBREAK','SUDDEN_DEATH','DRAW','FORFEIT'].map(x=>terminalPhaseForEndReason(x as any))).toEqual(Array(5).fill('FINISHED'));expect(['NO_CONTEST','NO_CONTEST_ASSET_LOAD'].map(x=>terminalPhaseForEndReason(x as any))).toEqual(['CANCELLED','CANCELLED']);});
