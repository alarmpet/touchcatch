...
begin
  ...
  if not exists (select 1 from pg_roles where rolname = 'account_worker') then create role account_worker nologin noinherit nosuperuser nocreatedb nocreaterole noreplication; end if;
  if not exists (select 1 from pg_roles where rolname = 'account_deletion_policy_role') then create role account_deletion_policy_role nologin noinherit nosuperuser nocreatedb nocreaterole noreplication; end if;
end
$$;

grant app_server, deployment_role, economy_deployment_role, economy_server, admin_publish_role, account_worker, account_deletion_policy_role to postgres;
