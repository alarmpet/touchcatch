...
select is((select count(*)::int from pg_proc where prosecdef and pronamespace in ('public'::regnamespace, 'private'::regnamespace) and oid not in (
  ...
  'private.ensure_account_v1(uuid)'::regprocedure,
  'private.read_me_v1(uuid)'::regprocedure,
  'private.resolve_published_learning_revision_v1(text,text)'::regprocedure,
  'private.merge_learning_progress_v1(uuid,uuid,text,jsonb)'::regprocedure
