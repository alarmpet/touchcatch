...
create or replace function private.publish_attested_content_revision_v1(
  p_idempotency_key text, p_request_hash text, p_attestation_hash text,
  p_public_content jsonb, p_private_solution jsonb, p_rights_manifest jsonb,
  p_public_canonical text, p_private_canonical text, p_rights_canonical text,
  p_actor_ref text, p_session_ref text, p_owner_id text
) returns uuid language plpgsql security definer set search_path = pg_catalog, public, private as $$
declare v_receipt private.admin_publish_receipts%rowtype; v_revision uuid; v_fence bigint;
begin
  ...
  insert into private.admin_publish_receipts(idempotency_key,request_hash,attestation_hash,owner_id,fence,lease_expires_at,state)
    values(p_idempotency_key,p_request_hash,p_attestation_hash,p_owner_id,1,clock_timestamp()+interval '30 seconds','PENDING')
  ...
  if v_receipt.owner_id<>p_owner_id and v_receipt.lease_expires_at>clock_timestamp() then raise exception 'PUBLISH_IN_PROGRESS'; end if;
  update private.admin_publish_receipts set owner_id=p_owner_id,fence=fence+1,lease_expires_at=clock_timestamp()+interval '30 seconds' where idempotency_key=p_idempotency_key returning fence into v_fence;
  ...
  update private.admin_publish_receipts set state='COMPLETED',content_revision_id=v_revision,result=jsonb_build_object('contentRevisionId',v_revision) where idempotency_key=p_idempotency_key and request_hash=p_request_hash and owner_id=p_owner_id and fence=v_fence;
...
grant select on private.admin_sessions to deployment_role;
grant insert on private.admin_publish_audit to deployment_role;
