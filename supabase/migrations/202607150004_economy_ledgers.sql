...
language plpgsql security definer set search_path=pg_catalog as $$declare policy private.economy_policy_revisions%rowtype; existing private.reward_ledger%rowtype; amount bigint; response jsonb; ledger_id bigint; opkey text; begin
  ...
  if found then if existing.committed_result_revision<>committed_result_revision then raise exception 'IDEMPOTENCY_CONFLICT'; end if; return existing.response_body; end if;
  ...
  insert into private.reward_ledger(match_id,subject_key,reward_type,committed_result_revision,request_hash,amount,economy_version,economy_hash,catalog_revision,catalog_hash,response_body) values(match_id,subject_key,reward_type,committed_result_revision,encode(extensions.digest(convert_to(jsonb_build_object('scope','REWARD_V1','matchId',match_id,'subjectKey',subject_key,'rewardType',reward_type,'committedResultRevision',committed_result_revision)::text,'UTF8'),'sha256'),'hex'),amount,policy.economy_version,policy.economy_hash,expected_catalog_revision,expected_catalog_hash,response) returning reward_ledger_id into ledger_id;
