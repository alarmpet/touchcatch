...
describe('PostgresWeeklyCategoryBoard', () => {
  it('calls the restricted aggregate and strictly parses public DTOs', async () => {
    const value = { seasonId: '30000000-0000-4000-8000-000000000001', category: 'ENGLISH', snapshotRevision: 'snapshot-1', rows: [{ rank: 1, nickname: 'One', displayScore: 500 }], myRank: { rank: 1, totalCompetitors: 1, percentile: 100, displayScore: 500 } };
    const rpc = { call: async () => value, callParsed: async <T>(_name: never, _args: readonly unknown[], parse: (input: unknown) => T) => parse(value) };
  ...
  it('rejects private fields returned by a compromised adapter', async () => {
    const value = { seasonId: '30000000-0000-4000-8000-000000000001', category: 'ENGLISH', snapshotRevision: 'x', rows: [{ rank: 1, nickname: 'One', displayScore: 1, subjectKey: 'private' }], myRank: null };
    const rpc = { call: async () => value, callParsed: async <T>(_name: never, _args: readonly unknown[], parse: (input: unknown) => T) => parse(value) };
