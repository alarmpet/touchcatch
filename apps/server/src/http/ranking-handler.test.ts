...
const attempts = { enabled: true as const, rulesetHash: 'd'.repeat(64), hintPolicyHash: 'e'.repeat(64), competitionPolicyHash: 'c'.repeat(64), catalogRevision: 'c', catalogHash: 'b'.repeat(64) };
...
describe('weekly ranking HTTP handler', () => {
  it('returns one strict authenticated board', async () => {
    ...
    const handler = createRankingHandler({ verifier: { verify: async () => ({ authenticatedUserId: '10000000-0000-4000-8000-000000000001' }) }, subjectResolver: { ensureAndResolve: async () => '20000000-0000-4000-8000-000000000001' }, getPolicy: () => ({ rewards: enabled, ranking: enabled, attempts }), board });
  ...
  it('maps malformed queries, disabled policies, and storage failures without leaking causes', async () => {
    ...
    const disabled = createRankingHandler({ verifier: { verify: async () => ({ authenticatedUserId: 'x' }) }, subjectResolver: resolver, getPolicy: () => ({ rewards: enabled, ranking: { enabled: false, code: 'RANKING_POLICY_NOT_APPROVED' }, attempts: { enabled: false, code: 'RANKING_POLICY_NOT_APPROVED' } }), board });
    ...
    const failing = createRankingHandler({ verifier: { verify: async () => ({ authenticatedUserId: 'x' }) }, subjectResolver: { ensureAndResolve: async () => 'y' }, getPolicy: () => ({ rewards: enabled, ranking: enabled, attempts }), board: { read: async () => { throw new Error('password=secret constraint_private'); } } });
