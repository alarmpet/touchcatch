...
const google = { id: 'google-id', user_id: 'user-id', identity_id: 'google-id', provider: 'google' };
const email = { id: 'email-id', user_id: 'user-id', identity_id: 'email-id', provider: 'email' };

it('verifies a server-issued reauthentication OTP before linking a fresh provider', async () => {
  const calls: string[] = [];
  const auth = {
    verifyOtp: vi.fn(async () => { calls.push('reauth'); return { data: null, error: null }; }),
...
it('opens and completes the identity-link PKCE callback then verifies fresh identities', async () => {
  ...
  const exchangeCodeForSession = vi.fn(async () => ({ data: null, error: null }));
  const coordinator = createIdentityCoordinator({ verifyOtp: async () => ({ data: null, error: null }), getUserIdentities, linkIdentity: async () => ({ data: { url: 'https://accounts.google.test' }, error: null }), unlinkIdentity: vi.fn(), exchangeCodeForSession }, { storage, browser: { openAuthSessionAsync: async () => ({ type: 'success', url: 'spotlearn://auth/callback?code=pkce-code' }) } });
...
it('deduplicates simultaneous browser and app callback completion', async () => {
  ...
  const exchangeCodeForSession = vi.fn(async () => { await Promise.resolve(); return { data: null, error: null }; });
...
it('requests the reauthentication nonce from Supabase instead of inventing client proof', async () => {
  const reauthenticate = vi.fn(async () => ({ data: null, error: null }));
...
it('uses fresh provider and identity IDs rather than email matching when unlinking', async () => {
  ...
  const unlinkIdentity = vi.fn(async () => ({ data: null, error: null }));
  const coordinator = createIdentityCoordinator({ verifyOtp: async () => ({ data: null, error: null }), getUserIdentities, linkIdentity: vi.fn(), unlinkIdentity });
...
it('refuses stale, unknown, or last viable identity removal', async () => {
  const base = { verifyOtp: async () => ({ data: null, error: null }), linkIdentity: vi.fn(), unlinkIdentity: vi.fn() };
