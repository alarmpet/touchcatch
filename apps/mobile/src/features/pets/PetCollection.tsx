...
export type PetItem = { id: string; name: string; rarity: 'COMMON' | 'RARE' | 'LEGENDARY'; ownedCopies: number; artUrl?: string };
...
export function PetCollection({ pets, totalCatalogCount, status = pets.length ? 'READY' : 'EMPTY', onPromotePet }: PetCollectionProps) {
  ...
  const ownedCount = pets.filter((p) => p.ownedCopies > 0).length;
  ...
  return <View style={{ padding: 14, backgroundColor: '#FFF', borderRadius: 8, borderWidth: 1, borderColor: '#E2E8F0', marginVertical: 8 }}>
    ...
    <Text style={{ fontSize: 14, color: '#0B7A75' }}>수집률 {completionPercentage}% ({ownedCount}/{totalCatalogCount})</Text>
    <ScrollView horizontal style={{ marginTop: 12 }}>{pets.map((pet) => <View key={pet.id} style={{ width: 110, padding: 8, borderWidth: 1, borderColor: '#E2E8F0', borderRadius: 6, marginRight: 8, alignItems: 'center' }}>
      ...
      <Text style={{ fontWeight: 'bold', color: '#17324D' }}>{pet.name}</Text><Text style={{ color: '#64748B' }}>{pet.rarity}</Text><Text style={{ color: '#475569' }}>보유: {pet.ownedCopies}개</Text>
      {pet.ownedCopies >= 11 && <Pressable accessibilityRole="button" accessibilityLabel={`${pet.name} 승급`} onPress={() => onPromotePet?.(pet.id)}><Text>승급 (10개)</Text></Pressable>}
    </View>)}</ScrollView>
  </View>;
}
