...
describe('analytics privacy boundary', () => {
  ...
  it('emits the shared real requirement gate metric with opaque ID and changing result',()=>{expect(emitRequirementGateStatus('RULE-001',true)).toEqual({metric:'release_requirement_gate_status',requirementId:'RULE-001',result:'PASS',value:1});expect(emitRequirementGateStatus('RULE-001',false)).toMatchObject({result:'FAIL',value:0});expect(()=>emitRequirementGateStatus('bad',true)).toThrow();});
});
