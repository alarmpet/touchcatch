...
describe('analytics privacy boundary', () => {
  ...
  it('emits the shared real requirement gate metric with PASS, FAIL, or BLOCKED result',()=>{expect(emitRequirementGateStatus('RULE-001','PASS')).toEqual({metric:'release_requirement_gate_status',requirementId:'RULE-001',result:'PASS',value:1});expect(emitRequirementGateStatus('RULE-001','FAIL')).toMatchObject({result:'FAIL',value:0});expect(emitRequirementGateStatus('DOC-001','BLOCKED')).toMatchObject({result:'BLOCKED',value:0});expect(()=>emitRequirementGateStatus('bad','PASS')).toThrow();});
});
