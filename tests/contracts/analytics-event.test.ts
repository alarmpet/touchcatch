...
import { AnalyticsCollector, parseAnalyticsEventV1, reconstructTrace, parseExperimentContractV1 } from '../../packages/contracts/src/analytics.js';
...
describe('analytics privacy boundary', () => {
  ...
  it('combines experiment schema item bounds with runtime sum parity',()=>{const schema=JSON.parse(fs.readFileSync('schemas/experiment-contract.v1.schema.json','utf8'));const validate=new Ajv2020({strict:false}).compile(schema),good={schemaVersion:1,experimentId:'battle-v1',assignmentUnit:'anonymousUserId',assignmentSaltVersion:'salt-v1',variants:['CONTROL','TREATMENT'],allocationBasisPoints:[5000,5000],primaryMetric:'final_attempt_rate',mde:.03,alpha:.05,power:.8,minSamplePerVariant:10000,guardrails:['tap_result_p95','unexpected_failure_rate','srm'],stoppingRule:'FIXED_HORIZON',cellBucket:'03:12'};expect(validate(good)).toBe(true);expect(()=>parseExperimentContractV1(good)).not.toThrow();for(const allocationBasisPoints of [[10000,0],[6000,3000]]){const bad={...good,allocationBasisPoints};expect(validate(bad)).toBe(allocationBasisPoints[1]!==0);expect(()=>parseExperimentContractV1(bad)).toThrow();}});
});
