...
it('publishes the approved HARD deletion admission and replay contract', () => {
  ...
  expect(api.components.schemas.AccountDeletionResponse.properties).toMatchObject({ status: { const: 'DELETING' }, policyPending: { const: false } });
});
