...
const entries = [
  ['ko-idiom-turn-misfortune', '전화위복'],
  ['ko-idiom-prepare-ahead', '유비무환'],
  ['ko-idiom-perspective', '역지사지'],
] as const;

describe('Korean idiom learning image pairs', () => {
  for (const [key, answer] of entries) {
    it(`${key} has a verified DRAFT bundle`, async () => {
      const paths = { a: resolve(root, 'source', `${key}-a.png`), b: resolve(root, 'source', `${key}-b.png`), bundle: resolve(root, 'drafts', `${key}.json`), evidence: resolve(root, 'evidence', `${key}.visual-delta.json`) };
      ...
      const challenges = [...bundle.privateSolution.wordHunts.map((item: { hitboxes: { imageA: { cx: number; cy: number; r: number } } }) => item.hitboxes.imageA), bundle.privateSolution.suddenDeath.hitboxes.imageA];
      for (const answerCircle of answers) for (const challengeCircle of challenges) expect(Math.hypot(answerCircle.cx - challengeCircle.cx, answerCircle.cy - challengeCircle.cy)).toBeGreaterThanOrEqual(answerCircle.r + challengeCircle.r);
    });
  }
});
